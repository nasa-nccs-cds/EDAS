...
object KernelPackageTools extends Loggable {
  ...
  def getKernelModuleMap( visibility: String = "" ): Map[String,KernelModule] = {
    ...
    val visible_kernels = ( internal_kernels ++ external_kernel_map ) filter { case (_,kmod) => kmod.nonEmpty }
    logger.info( " #PK# Visible Kernels Modules: " + visible_kernels.keys.mkString(", ") )
    visible_kernels
  }
