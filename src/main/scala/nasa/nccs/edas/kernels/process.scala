  ...
  override def map ( context: KernelContext ) (inputs: CDTimeSlice  ): CDTimeSlice = {
    ...
    if ( regrid_array_map.isEmpty ) {
      ...
      for ((uid, input_array) <- acceptable_array_map) {
        ...
        logger.info(s" #S# Sending acceptable Array ${uid} data to python worker, shape = [ ${input_array.shape.mkString(", ")} ], varRec=${optVarRec.fold(s"{gridfile=${targetGrid.gridFile}}")(_.toString)}, metadata = { ${data_array.metadata.toString} }")
      ...
      for ((uid, input_array) <- regrid_array_map) {
        ...
        logger.info(s" #S# Sending regrid Array ${uid} data to python worker, shape = [ ${input_array.shape.mkString(", ")} ], varRec=${optVarRec.fold("")(_.toString)}, metadata = { ${data_array.metadata.toString} }")
