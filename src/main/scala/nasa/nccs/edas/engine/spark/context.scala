...
//    logger.info( "EDAS env: \n\t" +  ( System.getenv.map { case (k,v) => k + ": " + v } ).mkString("\n\t") )
    ...
    Logger.getLogger("root").setLevel(log_level)
    Logger.getLogger("dag-scheduler-event-loop").setLevel(log_level)
    Logger.getLogger("org.apache.spark.scheduler.DAGScheduler").setLevel(log_level)
    Logger.getLogger("org.apache.spark.storage.BlockManager").setLevel(log_level)
    Logger.getLogger("org.apache.spark.scheduler.TaskSetManager").setLevel(log_level)
