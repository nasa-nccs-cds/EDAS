...
#                 self.logger.info( "### Receiving {0} bytes of data for trans_array, shape: {1}".format( trans_array.nbytes, ', '.join( map(str, trans_array.shape) )  ) )
#                 byte_data = self.data_socket.recv( trans_array.nbytes )
#                 self.logger.info( "### nbytes: {1}".format( str( len(byte_data) ) ) )
...
#         newVariable = newDataset.createVariable( resultId, np.float, variable.getAxisList() )
#         axes = [ axis.id for axis in variable.getAxisList() ]
#         newVariable.createattribute( 'order', variable.getOrder() )
#         newVariable.createattribute( 'axes', ",".join(axes) )
#         newDataset.close()
